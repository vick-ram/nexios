...
from nexios.auth import APIKeyAuthBackend, auth, BaseUser, AuthenticationMiddleware
...
async def test_apikey_auth_backend_success(test_client, api_key_data):
    ...
    class TestAPIKeyBackend(APIKeyAuthBackend):
        ...
        async def authenticate(self, request: Request, response: Response):
            ...
            if verify_key(raw_token, api_key_data["hashed_key"]):
                return AuthResult(success=True, identity="test_api_user", scope="apikey")
    ...
    async with client:
        response = await client.get("/protected", headers={"X-API-Key": api_key_data["api_key"]})
...
async def test_apikey_auth_backend_custom_header(test_client):
    ...
    async with client:
        response = await client.get("/protected", headers={"X-Custom-API-Key": "valid_custom_key"})
        assert response.status_code == 200
